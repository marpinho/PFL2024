...
% Players can either rotate a row or col or move a piece in each move. 
% This depends on the board configuration and on the next move type.

% valid_moves(+GameState, -ListOfMoves)
% Determines the list of valid moves based on the current phase.
valid_moves(game_state(Board, Pieces, CurrentPlayer, CurrentPhase, _RemainingMoves, _, _), ListOfMoves) :-
...
accessible(player1Exclusive, Player1) :-    % Exclusive squares for Player1.
    game_state(_, _, Player1, _, _, Player1, _).
accessible(player2Exclusive, Player2) :-    % Exclusive squares for Player2.
    game_state(_, _, Player2, _, _, _, Player2).
